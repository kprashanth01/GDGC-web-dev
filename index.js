...
function displayProducts(products) {
    ...
    products.forEach(product => {
        ...
        productItem.innerHTML = `<img src="${product.image}" alt="${product.title}">
            <h3>${product.title}</h3>
...
function viewCart() {
    ...
    cart.forEach(item => {
        ...
        cartItem.innerHTML = `<img src="${item.image}" alt="${item.title}"><span>${item.title}</span>
    ...
    const discount = (totalPrice > 100) ? 0.1 * totalPrice : 0;
    const platformFee = 10;
    const shippingCharges = 5;
    const finalAmount = totalPrice - discount + platformFee + shippingCharges;
...
function checkout() {
    alert(`Thank you for your purchase! You have been charged $${document.getElementById('final-amount').innerText}`);
    cart = [];
    updateCartCount();
    closeCart();
}
